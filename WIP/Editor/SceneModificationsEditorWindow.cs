...
namespace KorYmeLibrary.SceneModification
{
    public class SceneModificationsEditorWindow : EditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            if (_areParametersOpened = EditorGUILayout.Foldout(_areParametersOpened, "[PARAMETERS]", true, SceneModificationUtilities.ToRichText("foldout")))
            {
        ...
        private void InitTypes()
        {
            ...
            foreach (Assembly assembly in AppDomain.CurrentDomain.GetAssemblies())
            {
                ...
                {
                    foreach (MemberInfo member in type.GetMembers(SceneModificationUtilities.FLAGS_FIELDS))
                    {
        ...
        private void InitDictionary()
        {
            ...
            foreach (Type type in _allTypes)
            {
                ...
                foreach (Component component in allComponents)
                {
                    ...
                    foreach (MemberInfo member in component.GetType().GetMembers(SceneModificationUtilities.FLAGS_FIELDS))
                    {
        ...
        private void DisplayFirstObject()
        {
            foreach (Node node in _allNodes.Where(x => x._isOriginal).OrderBy(x => x._go.transform.GetSiblingIndex()))
            {
                ...
                if (_foldoutGameObject[node._go] = EditorGUILayout.Foldout(_foldoutGameObject[node._go],
                    SceneModificationUtilities.DisplayTextWithColours("[GAMEOBJECT] ", _colorGameObjectLabel) + node._go.name, true, SceneModificationUtilities.ToRichText("foldout")))
                {
                    EditorGUI.indentLevel++;
        ...
        private void DisplayChildGameObjects(List<Node> allChilds)
        {
            foreach (Node node in allChilds.OrderBy(x => x._go.transform.GetSiblingIndex()))
            {
                    ...
                    SceneModificationUtilities.DisplayTextWithColours("[GAMEOBJECT] ", _colorGameObjectLabel) + node._go.name, true, SceneModificationUtilities.ToRichText("foldout")))
                {
                    ...
                    {
                        EditorGUILayout.Space(2);
                        if (_foldoutComponent[component] = EditorGUILayout.Foldout(_foldoutComponent[component], SceneModificationUtilities.DisplayTextWithColours("[COMPONENT] ",
                            _colorComponentLabel) + component.GetType().ToString(), false, SceneModificationUtilities.ToRichText("foldout")))
                        {
                            EditorGUI.indentLevel++;
                            foreach (Tuple<FieldInfo,SceneModificationAttribute> tuple in _allComponentsInScene[node._go][component].Item2)
                            {
